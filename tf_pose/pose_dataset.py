...
class CocoPose(RNGDataFlow):
    ...
    def get_data(self):
        idxs = np.arange(self.size()) #total num of images
        if self.is_train:
            self.rng.shuffle(idxs) #shuffle the order of images to train
        else:
            pass

        keys = list(self.coco.imgs.keys()) # Use the coco api and get the ids of images
        for idx in idxs:
            img_meta = self.coco.imgs[keys[idx]] # get a image meta data
            img_idx = img_meta['id'] # get the image id 
            ann_idx = self.coco.getAnnIds(imgIds=img_idx) # get annotation indices on that image
            ...
            anns = self.coco.loadAnns(ann_idx) # get the image's annotation meta data
            meta = CocoMetadata(idx, img_url, img_meta, anns, sigma=8.0) 
            # data order(0~n), image path, img_meta(width, height), keypoints and num of them.
